...
public class BezelImageView extends ImageView {
    ...
    public void setMaskDrawable(Drawable drawable) {
        // We need to dereference our view from old mask callback before setting new mask
        if (mMaskDrawable != null) {
            mMaskDrawable.setCallback(null);
        }

        if (mMaskDrawable != null) {
            if (!mMaskDrawable.getConstantState().equals(drawable.getConstantState())) {
                // we should really change mask if only drawable is different
                mMaskDrawable = drawable;
                if (mMaskDrawable != null) {
                    mMaskDrawable.setCallback(this);
                }
            } else {
                // do nothing, mask is the same
                //L.d("Mask is the same, skipping");
            }
        }
    }
